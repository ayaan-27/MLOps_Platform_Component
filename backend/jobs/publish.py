...
'''
def publish(body_new):
    result = {"status": 0, "msg": "Some error occured!"}
    try:

        LOGGER.info("inside publish")
        
        job_type = body_new["type"]
        project_id = body_new["project_id"]
        user_id = body_new["user_id"]
        res = ds.get_ds_ver(project_id=project_id, user_id=user_id)
        if res["status"] !=0:
            dataset_id = res['msg']['dataset_id']
            version_id = res['msg']['version_id']

            body_new["dataset_id"] = dataset_id
            body_new["version_id"] = version_id
            LOGGER.info("Creating job entry in db")
            res_job = jm.make_job(dataset_id=dataset_id, version_id=version_id, project_id=project_id,
            ...
            if res_job["status"] != 0:
                ...
                body_new["job_id"] = res_job["msg"]

                channel.basic_publish(
                    'pacemlexchange', routing_key, json.dumps(body_new),
                    pika.BasicProperties(content_type='application/json'))
                print("Published")

                return {"status":1, "msg":{"status":"job pushed", "dataset_id": dataset_id, "version_id":version_id, "job_id": res_job["msg"]}}
            else:
                return result
        else:
            return result
