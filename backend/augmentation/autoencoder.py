...
class AutoencoderModel:
    ...
    def predict_with_noise_df(
        ...
        df_fake_with_noise = self.predict_df(
            no_samples, cols, scaler=scaler, target_class=target_class, cont_vars=cont_vars
        )
        if group_var:
            np_matrix = df_fake_with_noise.loc[:, df_fake_with_noise.columns != group_var].values
            np_matrix = np.array(
                [
                    val * (1 + np.random.normal(mu, sigma, 1))
                    for sublist in np_matrix
                    for val in sublist
                ]
            ).reshape(-1, np_matrix.shape[1])
